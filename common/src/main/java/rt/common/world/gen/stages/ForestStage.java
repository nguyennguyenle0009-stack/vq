...
public final class ForestStage implements ChunkStage {
  ...
  public ForestStage(WorldGenConfig cfg){ this.cfg = cfg; }

  @Override public void apply(int cx,int cy, Random rng, TileBuffer b){
    // TODO: thay bằng noise/rule của bạn
    for (int y=0;y<b.size;y++) for (int x=0;x<b.size;x++){
      if (b.getL1(x,y)==Terrain.PLAIN.id && rng.nextDouble()<0.08)
        b.setL1(x,y, Terrain.FOREST.id);
    }
  }
}
